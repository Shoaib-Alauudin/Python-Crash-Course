...
class myThread(threading.Thread):
    ...
    def __init__(self, name, ser):
        ...
        self.request_dict = {"a": '', "b": '', "c": '', "d": '', "e": '', "f": '', "g": '', "h": '', "k": '', "l": '',
                             "m": '', "n": ''}
...
def mQuit():
    ...
    mExit = messagebox.askyesno(title="Quit", message="Do you really want to quit?")
    if mExit > 0:
        # close port
        ser.close()
        # detsroy GUI
...
# headlines
mHeader = ttk.Label(root, text="CAN Monitoring Application").grid(row=0, column=1)
...
# Status table
a_Label = ttk.Label(root, text="Node 1 value").grid(row=row_start, column=0)
a_Show = ttk.Label(root, textvariable=a_var).grid(row=row_start, column=1)

b_Label = ttk.Label(root, text="Node 2 value").grid(row=row_start + 1, column=0)
